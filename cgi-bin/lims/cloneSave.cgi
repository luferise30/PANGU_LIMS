...
END
		...
		close(TMP);
		my $loadClones = $dbh->do("LOAD DATA LOCAL INFILE '$tmpdir/$$.clone' INTO TABLE clones (name, libraryId, plate, well, origin, sequenced)");
		my $toDelete = 1;
		do {
			$toDelete = 0;
			unlink("$tmpdir/$$.clone");
			$toDelete = 1 if (-e "$tmpdir/$$.clone");
			sleep 3;
		} while($toDelete);
		exit 0;
	}
