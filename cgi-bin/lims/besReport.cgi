...
if(param ('libraryId'))
{
	...
	for(@targetGenome)
	{
		...
		my $besToBeFlipped;
		open (BES,">$tmpdir/BES-$targetGenome->{$targetGenomeId}.txt") or die "can't open file: $tmpdir/BES-$targetGenome->{$targetGenomeId}.txt";
		...
		close (BES);
		`gzip -f $tmpdir/BES-$targetGenome->{$targetGenomeId}.txt`;
		...
		$mappedPairs .= ($mappedPairs) ? " and $paredBesMapped->{$targetGenomeId} ($besToBeFlipped->{$targetGenomeId} conflict)<a href='$tempurl/BES-$targetGenome->{$targetGenomeId}.txt.gz' target='hiddenFrame'><sup>$targetGenome->{$targetGenomeId}</sup></a>" : "$paredBesMapped->{$targetGenomeId} ($besToBeFlipped->{$targetGenomeId} conflict)<a href='$tempurl/BES-$targetGenome->{$targetGenomeId}.txt.gz' target='hiddenFrame'><sup>$targetGenome->{$targetGenomeId}</sup></a>";
	}
	$besDetails .= "</tr>";
	$besDetails .= "</tfoot></table>";

	open (BESTEXT,">$tmpdir/BES-report.$libraryId.html") or die "can't open file: $tmpdir/BES-report.$libraryId.html";
	print BESTEXT $besDetails;
	close (BESTEXT);
	`gzip -f $tmpdir/BES-report.$libraryId.html`;
	my $besReport = "<a href='$tempurl/BES-report.$libraryId.html.gz' target='hiddenFrame'>Download Details</a>" if (-e "$tmpdir/BES-report.$libraryId.html.gz");
