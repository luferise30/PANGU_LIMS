...
my $infile = "$tmpdir/$libraryId-$$.fpc";
...
END
		...
		{
			my $insertFpc=$dbh->prepare("INSERT INTO matrix VALUES (0, 'fpc', ?, ?, 0, 0, 0, ?, ?, ?, NOW())");
		...
		foreach my $clone (sort keys %$fpcClone)
		{
			...
			my $insertFpcClone = $dbh->prepare("INSERT INTO matrix VALUES (0,'fpcClone', ?, ?, ?, 0, 0, 0, ?, ?, NOW())");
		...
		foreach my $contig (sort keys %$fpcCtg)
		{
			...
			my $insertFpcCtg = $dbh->prepare("INSERT INTO matrix VALUES (0,'fpcCtg', ?, ?, ?, 0, 0, 0, ?, ?, NOW())");
