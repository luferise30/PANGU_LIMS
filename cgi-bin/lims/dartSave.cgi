...
END
									...
									foreach (@dartLine)
									{
										...
										else
										{
											if(!$dartSNPId)
											{
												...
												my $insertDartSNP=$dbh->prepare("INSERT INTO matrix VALUES (0, 'dartSNP', ?, 0, 0, 0, ?, 0, ?, ?, NOW())");
						...
						for (sort {$a <=> $b} keys %$genotype)
						{
							...
							my $insertDartGenotype=$dbh->prepare("INSERT INTO matrix VALUES (0, 'dartGenotype', ?, 0, 0, 0, ?, 0, ?, ?, NOW())");
...
END
	...
	else
	{
		if($dartFile || $dartFilePath)
		{
			...
			if($checkDartName->rows < 1)
			{
				my $insertDart=$dbh->prepare("INSERT INTO matrix VALUES (0, 'dart', ?, 0, 0, 0, ?, 0, ?, ?, NOW())");
