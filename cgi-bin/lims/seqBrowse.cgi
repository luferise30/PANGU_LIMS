...
# 	$refSequenceDetails->{'gapList'} = '' unless (exists $refSequenceDetails->{'gapList'});
	...
	my $totalLength = 0;
	my $besId = 0;
	open (BES,">$tmpdir/BES-$refSequence[2].txt") or die "can't open file: $tmpdir/BES-$refSequence[2].txt";
	...
	close (BES);
	`gzip -f '$tmpdir/BES-$refSequence[2].txt'`;
	my $besEvaluation = "<a href='$tempurl/BES-$refSequence[2].txt.gz' target='hiddenFrame'><span class='ui-icon ui-icon-bullet'></span>BES Evaluation</a>" if (-e "$tmpdir/BES-$refSequence[2].txt.gz");
	...
	open (SVGFILE,">$tmpdir/BES-$refSequence[2].svg") or die "can't open file: $tmpdir/BES-$refSequence[2].svg";
	print SVGFILE $seqDetails;
	close (SVGFILE);
	`gzip -f '$tmpdir/BES-$refSequence[2].svg'`;
	my $besSvg = "<a href='$tempurl/BES-$refSequence[2].svg.gz' target='hiddenFrame'><span class='ui-icon ui-icon-bullet'></span>SVG</a>" if (-e "$tmpdir/BES-$refSequence[2].svg.gz");
