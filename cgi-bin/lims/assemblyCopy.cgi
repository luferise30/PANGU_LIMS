...
if($assemblyId)
{
	...
	if($checkCreator[9] eq $userName || exists $userPermission->{$userId}->{'assembly'})
	{
		...
		$version++;
		my $insertAssembly=$dbh->prepare("INSERT INTO matrix VALUES (0, 'assembly', ?, ?, ?, ?, ?, ?, ?, ?, NOW())");
		...
		{
			my $insertAssemblySeq=$dbh->prepare("INSERT INTO matrix VALUES (0, 'assemblySeq', ?, ?, ?, ?, ?, ?, ?, ?, NOW())");
		...
		while(my @assemblyCtg = $assemblyCtg->fetchrow_array())
		{
			...
			my $insertAssemblyCtg=$dbh->prepare("INSERT INTO matrix VALUES (0, 'assemblyCtg', ?, ?, ?, ?, ?, ?, ?, ?, NOW())");
			...
			{
				my $insertComment=$dbh->prepare("INSERT INTO matrix VALUES (0, 'comment', ?, ?, ?, ?, ?, ?, ?, ?, NOW())");
