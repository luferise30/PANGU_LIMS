...
my $genomeInfile = "$tmpdir/$$.genome";
my $agpInfile = "$tmpdir/$$.agp";
		...
		if($checkGenomeName->rows < 1)
		{
			...
			if($checkCreator[9] eq $userName || exists $userPermission->{$userId}->{'genome'})
			{
				...
				if($agpFile) # since agp file is relatively small, we upload it first
				{
					...
					if($checkAgp->rows < 1)
					{
						my $insertAgp=$dbh->prepare("INSERT INTO matrix VALUES (0, 'agp', ?, ?, ?, ?, 0, 0, ?, ?, NOW())");
...
#							$seqDetails->{'sequence'} =~ s/^N+|N+$//g; #replace Ns at both ends.
							...
							my $insertSequence=$dbh->prepare("INSERT INTO matrix VALUES (0, 'sequence', ?, 99, ?, ?, ?, 0, ?, ?, NOW())");
							...
							if($split && @piece > 1)
							{
								...
								for(@piece)
								{
									...
									my $insertChildSequence=$dbh->prepare("INSERT INTO matrix VALUES (0, 'sequence', ?, 97, ?, ?, ?, ?, ?, ?, NOW())");
...
END
	...
	else
	{
		if($genomeFile || $genomeFilePath)
		{
			...
			if($checkGenomeName->rows < 1)
			{
				my $insertGenome=$dbh->prepare("INSERT INTO matrix VALUES (0, 'genome', ?, 0, ?, ?, ?, 0, ?, ?, NOW())");
