...
my $datasetInfile = "$tmpdir/$$.dataset";
...
END
						...
						while (<INFILE>) {
							...
							else
							{
								...
								my $insertTableRow=$dbh->prepare("INSERT INTO matrix VALUES (0, 'record', ?, 0, 0, 0, ?, 0, ?, ?, NOW())");
...
END
	...
	else
	{
		if($datasetFile || $datasetFilePath)
		{
			...
			if($checkDatasetName->rows < 1)
			{
				my $insertDataset=$dbh->prepare("INSERT INTO matrix VALUES (0, 'dataset', ?, ?, 0, 0, ?, 0, ?, ?, NOW())");
