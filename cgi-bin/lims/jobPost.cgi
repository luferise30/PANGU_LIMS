...
my $polished = $commoncfg->getProperty('POLISHED');
my $vector = $commoncfg->getProperty('VECTOR');
my $vectorlength = $commoncfg->getProperty('VECTORLENGTH')
...
my $jobdir = $commoncfg->getProperty('JOBDIR');
...
END
	...
	if(@vectorId)
	{
		...
		for(@vectorId)
		{
			unless ($_)
			{
				open (DEFAULTVECTOR,"$vector") or die "can't open file: $vector";
				while(<DEFAULTVECTOR>)
				{
					print VECTOR $_;
				}
				close(DEFAULTVECTOR);
				$vectorLength->{'pAGIBAC1.HindIII'} = $vectorlength;
				next;
			}
	...
	foreach my $input (sort {$a <=> $b} @jobId)
	{
		...
		{
			next if(!-e "$polished/$input.fasta.gz");
			`cp -fru $polished/$input.fasta.gz $outdir`;
		}
		...
		while ( my $seq = $in->next_seq() )
		{
			...
			my $insertSequence=$dbh->prepare("INSERT INTO matrix VALUES (0, 'sequence', ? , 0, ?, ?, 0, 0, ?, ?, NOW())");
			$insertSequence->execute($seq->id(),$input,$seqLength,$seqDetails,$userName);

			if($seqLength < $shortCutoff)
			{
				#ignore short sequences (type-8)
				$insertSequence=$dbh->prepare("INSERT INTO matrix VALUES (0, 'sequence', '', 8, ?, ?, 0, 0, ?, ?, NOW())");
			...
			for(sort {$a <=> $b} keys %breakPoint)
			{
				if($pieceLeft)
				{
					...
					if ($pieceLeft eq "SequenceEnd")
					{
						...
						{
							#no vector(type-3)
							$insertSequence=$dbh->prepare("INSERT INTO matrix VALUES (0, 'sequence', '', 3, ?, ?, 0, 0, ?, ?, NOW())");
						...
						elsif($breakPoint{$_} eq "VectorEnd")
						{
							...
							$insertSequence=$dbh->prepare("INSERT INTO matrix VALUES (0, 'sequence', '', 6, ?, ?, 0, 0, ?, ?, NOW())");
							$insertSequence->execute($input,$seqLength,$sequenceDetails,$userName);
					...
					elsif($pieceLeft eq "VectorEnd")
					{
						...
						{
							#mixer (type-7)
							$insertSequence=$dbh->prepare("INSERT INTO matrix VALUES (0, 'sequence', '', 7, ?, ?, 0, 0, ?, ?, NOW())");
							$insertSequence->execute($input,$seqLength,$sequenceDetails,$userName);
					...
					else # "InsertEnd"
					{
						...
						else # "InsertEnd"
						{
							...
							else
							{
								...
								$insertSequence=$dbh->prepare("INSERT INTO matrix VALUES (0, 'sequence', '', 1, ?, ?, 0, 0, ?, ?, NOW())");
								$insertSequence->execute($input,$seqLength,$sequenceDetails,$userName);
				...
				if(-e "$tmpdir/subSeqB$$")
				{
					...
					if($queryPosition > 0)
					{
						...
						if($subjectPosition == 1) #only subSeqA is used because alignment of the subSeqB start from 1.
						{
							...
							$sequenceDetails->{'gapList'} = "";
							$sequenceDetails = $json->encode($sequenceDetails);
							$insertSequence=$dbh->prepare("INSERT INTO matrix VALUES (0, 'sequence', '', 2, ?, ?, 0, 0, ?, ?, NOW())");
							$insertSequence->execute($input,$seqLength,$sequenceDetails,$userName);
					...
					else
					{
						if($pieceNumber > 99) #to be polished
						{
							...
							$insertSequence=$dbh->prepare("INSERT INTO matrix VALUES (0, 'sequence', '', 5, ?, ?, 0, 0, ?, ?, NOW())");
						...
						else
						{
							...
							{
								$insertSequence=$dbh->prepare("INSERT INTO matrix VALUES (0, 'sequence', '', 4, ?, ?, 0, 0, ?, ?, NOW())");
								$insertSequence->execute($input,$seqLength,$sequenceDetails,$userName);
								$sequence = ">" . $seq->id(). " (subSeqB:$subSeqBLeft-$subSeqBRight,100Ns,subSeqA:$subSeqALeft-$subSeqARight)\n" .
											$seq->subseq($subSeqBLeft,$subSeqBRight) . "N" x 100 . $seq->subseq($subSeqALeft,$subSeqARight) ."\n";
